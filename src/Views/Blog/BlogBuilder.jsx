...
const BlogBuilder = ({ action = 'add' }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      let response;
      if (action === 'add') {
        response = await api.addBlog(payload);
      } else {
        response = await api.updateBlog(id, payload);
      }

      if (response && response.isExecute === 'SUCCESS') {
        toast.success(`Blog ${action === 'add' ? 'created' : 'updated'} successfully!`);
              ...
              <div className="mt-4 mb-5 pb-5">
                ...
                >
                  {isSubmitting ? (
                    ...
                    <><i className="fa fa-save me-2"></i> {action === 'add' ? 'Create Blog' : 'Update Blog'}</>
                  )}
                </button>
                <button
